...
//				public void run() {
//					do {
//					} while (Dungeon.getDungeon().getCurrentRaum()
//							.isLeaveRoom() == false);
//				}
//			};
//
...
//			thread.start();
			...
			case Command.NIMM:
				if (args > 1) { // (ACHTUNG: auch bei "nimm blauen hut" wird
					...
					case "aus truhe":
						...
						if (truhe != null) {
							doTakeFromChest(truhe.getInventory()
									.findItemByName(parsed_args[0]));
						} else {
	...
	public static void printText(String text) {
		if (Textie.diag == true) {
			System.out
					.println(Dungeon.getDungeon().getCurrentRaum() == null ? text
							: "["
									+ Dungeon.getDungeon().getCurrentRaum().roomNumber
									+ "], " + text);
		} else {
		...
		// Thread yield() singnalisiert das der Thread in den hintergrund geschoben werden kann.
		Thread.yield();
	}
	...
	static void doGehen(Richtung richtung) {
		if (Dungeon.getDungeon().getCurrentRaum().getNumber() == 6
				&& Dungeon
						...
						.getCurrentRaum()
						.getNextRoom(
								Dungeon.getDungeon().getCurrentRaum()
										.findDoorByDirection(richtung)) == null) {
			printText("Der Weg wird durch eine Holzbarrikade versperrt.");
		} else {
			Raum raum = Dungeon.getDungeon().getRaum(richtung);
			if (raum != null
					&& Dungeon.getDungeon().raums.get(
							Dungeon.getDungeon().previousRoomNumber)
							.isLeaveRoom()) {
				Dungeon.getDungeon().setRoomNumber(raum);
				Dungeon.getDungeon().getCurrentRaum().setLeaveRoom(false);
				Textie.printText(Dungeon.getDungeon().getCurrentRaum()
						.getWillkommensNachricht());
			}
		}
	 ...
	 * 
	 * @param item
	 *            The item to throw away.
	 * @param count
	 *            The size of the parsed_command String[]
	 */
	static void doVernichte(Item item, int count) {
		if (count == 2) {
			if (Dungeon.getDungeon().player.getInventory().transferItem(
					Dungeon.getDungeon().getCurrentRaum().getInventory(), item)) {
		...
		if (count == 2) {
			...
			case "raum":
				if (Dungeon.getDungeon().getCurrentRaum().getNumber() == 3) {
					...
					Textie.printText("Im Raum befindet sich:");
					Dungeon.getDungeon().getCurrentRaum().getInventory()
							.listItems();
				}
				break;
			case "inventar":
				if (Dungeon.getDungeon().getCurrentRaum().getNumber() == 3) {
					...
					if (item instanceof ToggleItem) {
						...
						if (fackel.getState() == true) {
							...
							Dungeon.getDungeon().player.getInventory()
									.listItems();
						} else {
							printText("Du kannst nichts sehen!");
						}
				...
				break;
			case "truhe":
				if (Dungeon.getDungeon().getCurrentRaum().getInventory()
						.hasItem("Truhe")) {
					StorageItem truhe = (StorageItem) Dungeon.getDungeon()
							.getCurrentRaum().getInventory()
							.findItemByName("Truhe");
			...
			default:
				if (Dungeon.getDungeon().getCurrentRaum().getNumber() == 3) {
					Item item = Dungeon.getDungeon().getCurrentRaum()
							.getInventory().findItemByName("Fackel");
					if (item instanceof ToggleItem) {
						ToggleItem fackel = (ToggleItem) item;
						if (fackel.getState() == true) {
							Item itemUSU = Dungeon.getDungeon()
									.getCurrentRaum().getInventory()
									.findItemByName(parsed_command[1]);
					...
					Item itemUSU = Dungeon.getDungeon().getCurrentRaum()
							.getInventory().findItemByName(parsed_command[1]);
					Item itemUSU1 = chooseInventory(parsed_command[1]);
	...
	static void doBenutze(Item item) {
		if (item == null) {
			...
			Inventory raumInventar = Dungeon.getDungeon().getCurrentRaum()
					.getInventory();
			Inventory playerInventory = Dungeon.getDungeon().player
					.getInventory();
			if (item.isPickable() == false
					|| playerInventory.hasItem(item.getName())) {
				...
				// aufrufen
				// und nicht nur einen Text ausgeben. Außerdem sollen diese
				// Items
				...
				case "Feuerzeug": // Dungeon.getDungeon().itemMap.get("FEUERZEUG").getName():
					int fackelSlot = playerInventory.findItem(playerInventory
							.findItemByName("Fackel"));
					int feuerZeugSlot = playerInventory
							.findItem(playerInventory
									.findItemByName("Feuerzeug"));
					if (feuerZeugSlot < 0) {
				...
				case "Axt":
					Item axt = item;
					if (Dungeon.getDungeon().getCurrentRaum().getNumber() == 6) {
						Dungeon.getDungeon().getCurrentRaum().getDoors()
								.add(new Door(11, Richtung.OST, 7, false));
						axt.benutzen();
					} else {
				...
				case "Sack":
					Item sack = item;
					sack.benutzen();
					playerInventory.getInventory().remove(
							playerInventory.findItemByName("Sack"));
				...
				case "Schalter":
					...
					Dungeon.getDungeon().doorSchalter.get(schalter)
							.toogleLock();
					break;
				case "Schwert":
					...
					break;
				case "Schlüssel":
					StorageItem truhe = (StorageItem) raumInventar
							.findItemByName("Truhe");
	...
	public static void doSpeichernJSON() {
		OutputStream buffer = null;
		try {
			ObjectMapper mapper = new ObjectMapper();
			buffer = new BufferedOutputStream(new FileOutputStream(
					"savegame.save"));
			mapper.writeValue(buffer, Dungeon.getDungeon());
		} catch (IOException e) {
			e.printStackTrace();
		} finally {
			if (buffer != null) {
	...
	public static void doLaden() {
		...
		/*
		 * savegame = IOUtils.readFromFile(); if (savegame != null) { Dungeon
		 * loadedDungeon = new
		 * JSONDeserializer<Dungeon>().deserialize(savegame);
		 * Dungeon.setDungeon(null);
		 * Dungeon.getDungeon().player.setInventory(null);
		 * Dungeon.setDungeon(loadedDungeon);
		 * //Dungeon.getDungeon().player.setInventory
		 * (loadedDungeon.player.getInventory()); printText("Geladen!");
		 * //printText("Raum: " + dungeon1.currentRaum.getNumber());
		 * printText("Raum:" +
		 * Dungeon.getDungeon().getCurrentRaum().getNumberAsString()); } else {
	...
	public static boolean addItemFromChestToInventory(Item item) {
		StorageItem dieTruhe = (StorageItem) Dungeon.getDungeon()
				.getCurrentRaum().getInventory().findItemByName("Truhe");
		if (Dungeon.getDungeon().player.getInventory().getInventory().size() < Dungeon
				.getDungeon().player.getInventory().getInventorySize()
				&& dieTruhe.getInventory().hasItem(item.getName())) {
			dieTruhe.getInventory().transferItem(
					Dungeon.getDungeon().player.getInventory(), item);
			return true;
		} else {
			return false;
		}
	...
	public static boolean giveItem(Item item) {
		if (Dungeon.getDungeon().player.getInventory().getInventory()
				.remove(item)) {
			return true;
		}
		return false;
	...
	static void doReden() {
		if (Dungeon.getDungeon().getCurrentRaum().getHuman().isQuestDone() == true) {
			if (Dungeon.getDungeon().getCurrentRaum().getHuman().isGaveItem() == true) {
				if (recieveItem(Dungeon.getDungeon().getCurrentRaum()
						.getHuman().getRewarditem())) {
					printText("Hier, bitte schön.");
					Dungeon.getDungeon().getCurrentRaum().getHuman()
							.setGaveItem(false);
				} else {
				...
				switch (dialogNumber) {
				case 0:
					printText(Dungeon.getDungeon().getCurrentRaum().getHuman()
							.getDialog1());
					dialogNumber = 1;
					break;
				case 1:
	...
	public static void doGeben(String[] parsed_command, int count) {
		if (count == 2) {
			...
			// Item itemToUse = chooseInventory(parsed_command[1]);
			if (itemToUse.equals(Dungeon.getDungeon().getCurrentRaum()
					.getHuman().getQuestItem())) {
	...
	public static Item chooseInventory(String itemName) {
		...
		if (Dungeon.getDungeon().player.getInventory().findItemByName(itemName) != null) {
			item = Dungeon.getDungeon().player.getInventory().findItemByName(
					itemName);
		} else if (Dungeon.getDungeon().getCurrentRaum().getInventory()
				.findItemByName(itemName) != null) {
			item = Dungeon.getDungeon().getCurrentRaum().getInventory()
