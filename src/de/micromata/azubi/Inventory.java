...
public class Inventory {
	...
	/*
	 * public boolean addToInventory(int objectID, Raum raum) { List <Item>items
	 * = raum.items; int objektInUmgebung = -128; for (int i = 0; i <
	 * MAX_SLOTS_INVENTORY; i++) { if (inventory[i] == 0) { inventory[i] =
	 * objectID; for (int y = 0; y < items.size(); y++) { if (
	 * items.get(y).getID() == objectID) { objektInUmgebung = i; } } if
	 * (objektInUmgebung != -128) { raum.removeItem(objectID); return true; } }
	 * } return false; }
	 */
	...
	/*
	 * public void setItemForStartHack(int item, int slot){ = item; }
	 * 
	 * public void setAlive(boolean par){ this.alive = par; }
	 */
